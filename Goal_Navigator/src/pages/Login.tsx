...
const Login = () => {
  ...
  const validateEmail = (email: string) => {
    return email.endsWith("@bia.app") || email.endsWith("@bia.energy");
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateEmail(email)) {
      setError("Solo se permiten correos corporativos @bia.app");
      return;
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4">
      <Card className="w-full max-w-md border-border/50 shadow-lg">
        <CardHeader className="text-center pb-2">
          ...
          <h1 className="text-2xl font-bold text-foreground">People Space</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Plataforma de gestión de talento de Bia
          </p>
        </CardHeader>
            ...
            <Button
              ...
              onClick={async () => {
                ...
                const result = await lovable.auth.signInWithOAuth("google", {
                  ...
                  extraParams: { hd: "bia.app", prompt: "select_account" },
                });
                if (result.error) {
